...
//ELAINE -- baron_ref(state, choice1, currentPlayer);
//CHUNG -- int baronR(int choice1, int currentPlayer, struct gameState *state, int *bonus) 
        baronR(rand_num_pos, p, &G, 0);  
    }
    printf("\n");
...
printf("\nRandom test of current-player input with choice1 as 1\n");
    for(i = 0; i < n; i++)
    {
        ...
        baronR(rand_num_pos, choice, &G, 0);  
    }
    printf("\n");
