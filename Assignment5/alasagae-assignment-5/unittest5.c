...
//TEST 2: TRY TO BUY MINE - COST = 5
        ...
        int before_action = G.numActions;
...
//ELAINE -- int mine_ref(struct gameState *state, int choice1, int choice2, int currentPlayer, int handPos)
//CHUNG -- int mineR(int choice1, int choice2, int currentPlayer, struct gameState *state, int handPos);
        if(mineR(5, 6, p, &G, 1) == 0)
        {
...
//TEST 4: EXECUTE REFACTORED MIN//TEST 3:   EXECUTE REFACTORED MINE FUNCTION
        ...
        if(mineR(hand_pos_card, 6, p, &G, hand_pos_card) == 0)
        {
            printf("    PASS: %s card executed\n", cardName);
        }
        else
...
//TEST 5:   CHECK IF TRASHED CARD WAS REMOVED FROM DECK AND HAND
        printf("---------------------------------------\n");
        printf("TEST 5: CHECK IF TRASHED CARD WAS REMOVED FROM DECK AND HAND\n");
        int confirm_trash = 0;
...
//TEST 6:   CHECK IF TRASHED CARD WAS REMOVED FROM DECK AND HAND
        printf("---------------------------------------\n");
        printf("TEST 6: CHECK IF GAINED CARD IS IN HAND OR DECK\n");
