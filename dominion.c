...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
        return mineR(choice1, choice2, currentPlayer, state, handPos);
    ...
    case baron:
        return baronR(choice1, currentPlayer, state, bonus);
    ...
    case minion:     
        return minionR(choice1, choice2, currentPlayer, state, handPos, bonus);
    ...
    case tribute:
        return tributeR(currentPlayer, nextPlayer, tributeRevealedCards, state, bonus);

    case ambassador: 
        return ambassadorR(choice1, choice2, currentPlayer, state, handPos);
...
int baronR(int choice1, int currentPlayer, struct gameState *state, int *bonus) 
{
    ...
    state->numBuys++; //introduce bug 1: make incorrect buys number
    if (choice1 > 0) { //Boolean true or going to discard an estate
        ...
        while(card_not_discarded) {
            if (state->hand[currentPlayer][p] == estate) { //Found an estate card!
                ...
                state->coins -= 1; //introduce bug 2, make coins value not correct
...
int minionR(int choice1, int choice2, int currentPlayer, struct gameState *state, int handPos, int *bonus)
{
    ...
    //+1 action
    state->numActions++;
    state->numActions = state->numActions +1; //introduce bug 1: make action counts not correct
    ...
    {
        state->coins = state->coins + 2;
        state->coins = state->coins + 1; //introduce bug 2: make incorrect  coin values
    }
...
int ambassadorR(int choice1, int choice2, int currentPlayer, struct gameState *state, int handPos)
{
    ...
    if (choice1 != handPos) //introduce bug 1: correct one should be using ==, this cuase failed if player choose copies
    {
        return -1;
    }
    ...
    state->supplyCount[state->hand[currentPlayer][choice1]] += 1; //introduce bug 2: make incorrect amount  supply cards
...
int tributeR(int currentPlayer, int nextPlayer, int tributeRevealedCards[], struct gameState *state, int *bonus)
{
    state->numActions++; //introduce bug 1: should not increase actions
    state->numBuys++; //introduce bug 2: should not increase buys
