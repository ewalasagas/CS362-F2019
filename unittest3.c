...
//TEST 2: TRY TO BUY AMBASSADOR - COST = 3
        ...
        int supply_count_before = supplyCount(0, &G);
        ...
        int hand_pos_cardcopy = 1;
        //introduce bug 1: correct one should be using ==, this cuase failed if player choose copies
        printf("---------------------------------------\n");
        printf("TEST BUG1:  CHOICE1 AND HANDPOS ARE THE SAME - TEST SHOULD RETURN -1\n");
        //Checking if supplyCount of card to copy is greater than 0 -- supplyCount(choice1, state)
        if(ambassadorR(hand_pos_cardcopy, 1, p, &G, hand_pos_cardcopy) == -1) 
        {
            printf("    FAIL: Choice 1 and hand position are equal, and returns -1\n");
        }
        else
        {
            printf("    PASS: Choice 1 and hand position are NOT equal - executing per bug placement\n");
        }
...
//RESET INITIALIZATION OF GAME
        ...
        //cardname of chosen card to copy
        cardCopy[MAX_STRING_LENGTH];
        strcpy(cardCopy,"");
        cardNumToName(0, cardCopy);  

        curse_count_before = supplyCount(0, &G);
...
//TEST 3:   EXECUTE REFACTORED AMBASSADOR FUNCTION
     printf("---------------------------------------\n");
        ...
        hand_pos_cardcopy = 0;      //reset variable to ensure position
...
//ELAINE -- int ambassador_ref(int choice1, int choice2, struct gameState *state, int handPos, int currentPlayer)
//CHUNG -- int ambassadorR(int choice1, int choice2, int currentPlayer, struct gameState *state, int handPos)

        //Making 1 copies of curse
        if(ambassadorR(1, 1, p, &G, hand_pos_cardcopy) == 0) 
        {
...
//introduce bug 2: make incorrect amount  supply cards
        printf("---------------------------------------\n");
        printf("TEST 4:  CHECK SUPPLY COUNT OF COPIED CARD IS DECREMENTED BY 2 IF NUMBER PLAYERS IS 4\n");
        //Checking if supplyCount of card to copy is greater than 0 -- supplyCount(choice1, state)
        if((supplyCount(0, &G)-3) == supply_count_before)
        {
            printf("    PASS: Card %s supply count was decremented by 2 \n", cardCopy);
            printf("    Supply count before is %d, and after call is %d\n", supply_count_before, supplyCount(0, &G));
        }
        else
        {
